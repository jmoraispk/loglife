...
export default function DashboardPage() {
  ...
  const handleStartLinking = async () => {
    ...
    try {
      ...
      if (!regRes.ok) {
        setVerifyFeedback({ type: "error", text: regData.error || "Registration failed" });
        return;
      }
      if (!regData.linkCode) {
        setVerifyFeedback({
          type: "error",
          text: "Failed to generate link code. Restart gateway and try again.",
        });
        return;
      }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <main className="flex-1 flex flex-col gap-6 px-3 pb-6 pt-20 sm:px-4 lg:px-6 lg:pb-8 lg:pt-6 [&_button]:cursor-pointer">
        ...
        {sessionLoading ? (
          ...
          <div className="space-y-8">
            <div className="text-center max-w-xl mx-auto">
              ...
              <p className="text-sm text-slate-400 mt-2">
                Enter your phone number, then send the generated code from WhatsApp to link your dashboard.
              </p>
            </div>
                ...
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {verifyStep === "phone" ? "Step 1 of 2 — enter your number" : "Step 2 of 2 — send code in WhatsApp"}
                    </p>
                  </div>
                </div>
                    ...
                    <>
                      ...
                      {waTarget && linkCode ? (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-white bg-[#25D366] border border-[#35df79] hover:bg-[#20bd5a] hover:shadow-[0_0_20px_rgba(37,211,102,0.45)] hover:scale-[1.01] transition-all"
                        >
                          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                          </svg>
          ...
          <section className="grid grid-cols-12 gap-6">
            <div className="col-span-12 lg:col-span-3">
              <div className="rounded-2xl border border-white/10 bg-white/5 flex flex-col gap-4 p-4">
                ...
                <p className="text-sm text-slate-400">WhatsApp connected</p>
              </div>
            </div>
            <div className="col-span-12 lg:col-span-3">
              <div className="rounded-2xl border border-white/10 bg-white/5 flex flex-col gap-4 p-4">
                <p className="text-xs uppercase tracking-wide text-slate-500">Total Tokens</p>
                <p className="text-2xl font-semibold text-white">{formatTokens(session?.totalTokens)}</p>
            ...
            <div className="col-span-12 lg:col-span-3">
              <div className="rounded-2xl border border-white/10 bg-white/5 flex flex-col gap-4 p-4">
                ...
                <p className="text-2xl font-semibold text-white">{session?.model || "N/A"}</p>
            ...
            <div className="col-span-12 lg:col-span-3">
              <div className="rounded-2xl border border-white/10 bg-white/5 flex flex-col gap-4 p-4">
                ...
                <p className="text-2xl font-semibold text-white">
                  {session?.abortedLastRun ? <span className="text-red-400">Error</span> : <span className="text-emerald-400">Active</span>}
                </p>
                <p className="text-sm text-slate-400">Last active {formatRelativeTime(session?.updatedAt)}</p>
              </div>
            </div>
            ...
            <div className="col-span-12 min-h-[260px] lg:min-h-[320px]">
              <GoalsSection />
            </div>
          </section>
        ) : (
