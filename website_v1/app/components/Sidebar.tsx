...
export default function Sidebar() {
  ...
  // Signed-in: main nav (Home, Features, Blog, Pricing, Docs) then divider then user nav (Dashboard, Account)
          ...
          {isSignedIn ? (
            <>
              {authNavItemsMain.map((item, index) => (
                <Link key={index} href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel={item.href.startsWith("http") ? "noopener noreferrer" : undefined} className={`h-9 flex items-center gap-2 px-2 sm:px-3 rounded-lg transition-colors ${isActive(item.href) ? "text-emerald-400 bg-emerald-500/10" : "text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10"}`}>
                  {item.icon}
