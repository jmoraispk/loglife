...
def test_send_emulator_message_json_failure() -> None:
    ...
    with (
        patch("loglife.core.transports.log_broadcaster") as mock_broadcaster,
        # Pass a bad attachment that causes json.dumps to crash
        # But wait, json.dumps is only called if "transcript_file" is in
        # attachments. We can't easily force json.dumps to fail with simple
        # strings unless we mess up the dict heavily. But we can mock
        # json.dumps to raise exception
        patch("loglife.core.transports.json.dumps", side_effect=TypeError("Fail")),
    ):
