...
# Test signing key constant (not a real secret - only used in tests)
TEST_SIGNING_KEY = "test_secret_key"


def test_generate_short_token() -> None:
    """Test token generation."""
    phone = "+1234567890"
    token = generate_short_token(phone, TEST_SIGNING_KEY)
    ...
    token2 = generate_short_token(phone, TEST_SIGNING_KEY)
    assert token == token2


def test_generate_short_token_different_phones() -> None:
    """Test that different phones generate different tokens."""
    token1 = generate_short_token("+1234567890", TEST_SIGNING_KEY)
    token2 = generate_short_token("+9876543210", TEST_SIGNING_KEY)
...
def test_generate_short_token_different_secrets() -> None:
    ...
    phone = "+1234567890"
    token1 = generate_short_token(phone, TEST_SIGNING_KEY)
...
def test_generate_short_token_custom_length() -> None:
    ...
    phone = "+1234567890"
    token = generate_short_token(phone, TEST_SIGNING_KEY, length=16)
...
def test_validate_short_token_valid() -> None:
    ...
    assert validate_short_token(token, phone, TEST_SIGNING_KEY) is True
...
def test_validate_short_token_invalid_phone() -> None:
    ...
    assert validate_short_token(token, "+9999999999", TEST_SIGNING_KEY) is False
...
def test_validate_short_token_previous_window() -> None:
    ...
    with patch("loglife.core.tokens.time.time") as mock_time:
        ...
        assert validate_short_token(token, phone, TEST_SIGNING_KEY) is False
...
def test_token_cache_cleanup() -> None:
    ...
    # Generate two tokens
    token1 = generate_short_token(phone1, TEST_SIGNING_KEY)
    token2 = generate_short_token(phone2, TEST_SIGNING_KEY)
...
def test_generate_short_token_normalizes_phone() -> None:
    ...
    token1 = generate_short_token(phone_with_suffix, TEST_SIGNING_KEY)
    generate_short_token(phone_normalized, TEST_SIGNING_KEY)  # Generate but don't use
