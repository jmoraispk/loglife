...
def test_get_goal() -> None:
    ...
    retrieved_goal = db.goals.get(created_goal.id)
    ...
    non_existent_goal = db.goals.get(999)
...
def test_get_user_goals() -> None:
    ...
    user = db.users.create("+1234567890", "America/New_York")
    db.goals.create(user.id, "🎯", "Learn Python")
    db.goals.create(user.id, "💪", "Exercise daily")
    db.goals.create(user.id, "📚", "Read books")
    ...
    for goal in goals:
        assert goal.user_id == user.id
        assert goal.id is not None
        assert goal.goal_emoji is not None
        assert goal.goal_description is not None
        assert goal.boost_level is not None
        assert goal.created_at is not None

    # Test retrieving goals for user with no goals
    user2 = db.users.create("+9999999999", "Europe/London")
    empty_goals = db.goals.get_by_user(user2.id)
...
def test_update_goal() -> None:
    ...
    updated_goal = db.goals.update(goal.id, goal_emoji="🔥")
    ...
    updated_goal = db.goals.update(goal.id, goal_description="Master Python")

    assert updated_goal.goal_emoji == "🔥"
    assert updated_goal.goal_description == "Master Python"

    # Test updating boost_level only
    updated_goal = db.goals.update(goal.id, boost_level=2)
...
def test_delete_goal() -> None:
    ...
    # Verify goal exists
    assert db.goals.get(goal_id) is not None
