...
<Warning>
  Make sure your server's port 18789 is reachable from Vercel's servers. If you're behind a firewall, consider using a reverse proxy (Nginx, Caddy) or a tunnel (Tailscale Funnel, Cloudflare Tunnel).
</Warning>

### Plugin (server)

A GitHub Actions workflow triggers on pushes to `main` that change files in `plugin/**`. The workflow SSHes into your server and runs `git pull` to update the plugin code.

After the pull, restart the gateway to load the changes:
